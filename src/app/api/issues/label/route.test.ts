...
function postRequest(payload = makePayload()) {
  return POST(
    authedRequest("http://localhost/api/issues/label", {
      method: "POST",
      body: payload,
    }),
  );
...
describe("POST /api/issues/label", () => {
  ...
  it("accepts the bridge issueNumber payload and applies the label", async () => {
    const response = await postRequest();

    expect(response.status).toBe(200);
    expect((await response.json()).labels).toContain("needs-human");
    expect(mocks.addIssueLabel).toHaveBeenCalledWith("org/repo", 42, "needs-human");
  });
